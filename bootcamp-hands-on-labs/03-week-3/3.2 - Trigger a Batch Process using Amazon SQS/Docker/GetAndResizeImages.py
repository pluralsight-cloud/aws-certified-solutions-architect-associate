...
def process_images():
    ...
    for message in get_messages_from_sqs():
        try:
            ...
            image = urllib.unquote_plus(
                message_content["Records"][0]["s3"]["object"]["key"]
            ).encode("utf-8")
...
def upload_image(image):
    ...
    s3.upload_file(resized_dir + "/" + image, output_bucket_name, "resized/" + image)
    s3.upload_file(thumb_dir + "/" + image, output_bucket_name, "thumbs/" + image)
...
def get_messages_from_sqs():
    ...
    for message in queue.receive_messages(
        VisibilityTimeout=120, WaitTimeSeconds=20, MaxNumberOfMessages=10
    ):
